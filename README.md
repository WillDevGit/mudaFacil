...
Não é necessário Docker, Supabase CLI ou qualquer serviço externo.
...
| Erro                                                | Causa / solução                                                         |
| --------------------------------------------------- | ----------------------------------------------------------------------- |
| `Não foi possível conectar à API em http://...`      | O backend não está rodando (`cd backend && npm run dev`)                 |
| `password authentication failed for user "postgres"` | Ajuste `DB_USER`/`DB_PASSWORD` em `backend/.env`                          |
| `database "mudafacil" does not exist`                | Execute `createdb mudafacil`                                             |
| `relation "clientes" does not exist`                 | Rode `psql -d mudafacil -f database/schema.sql`                          |
| `EADDRINUSE :3001`                                   | Porta ocupada — altere `PORT` no `.env` e `VITE_API_URL` no frontend      |
| Telas vazias sem erro                                | Rode o `database/seed.sql`                                               |